...
// Derive firmware revision string from Bluetooth major, minor and patch
// versions.
...
// Check if the default values fit into the characteristic value buffers.
    ...
    defined(gattdb_firmware_revision_string_len) &&                            \
    defined(FIRMWARE_REVISION_STRING)
#define FIRMWARE_REVISION_STRING_LEN (sizeof(FIRMWARE_REVISION_STRING) - 1)
static_assert(
    gattdb_firmware_revision_string_len >= FIRMWARE_REVISION_STRING_LEN,
    "Bluetooth stack version does not fit into firmware revision string "
    "characteristic. Please adjust GATT configuration."
);
static const uint8_t firmware_revision_string[] = FIRMWARE_REVISION_STRING;
#endif

#if defined(gattdb_model_number_string) &&                                     \
    defined(gattdb_model_number_string_len) && defined(MODEL_NUMBER_STRING)
#define MODEL_NUMBER_STRING_LEN (sizeof(MODEL_NUMBER_STRING) - 1)
static_assert(
    gattdb_model_number_string_len >= MODEL_NUMBER_STRING_LEN,
    "Board name does not fit into model number string characteristic. Please "
    "adjust GATT configuration."
);
static const uint8_t model_number_string[] = MODEL_NUMBER_STRING;
#endif

#if defined(gattdb_hardware_revision_string) &&                                \
    defined(gattdb_hardware_revision_string_len) &&                            \
...
static_assert(
    ...
    "Please adjust GATT configuration."
);
...
static_assert(
    gattdb_system_id_len == 8,
    "System ID does not fit into the System ID characteristic. Please adjust "
    "GATT configuration."
);
#endif
...
// GATT Configurator to set the value manually.
    ...
    defined(HARDWARE_REVISION_STRING)
		sc = sl_bt_gatt_server_write_attribute_value(
		    gattdb_hardware_revision_string,
		    0,
